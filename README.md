...
| **Persistence** | 25+ mechanisms | Config inventory + scored command lines: cron (+ `cron.{hourly,daily,weekly,monthly}` & anacron), systemd units/timers (system+user), authorized_keys, rc.local (+RHEL) & init.d, ld.so.preload, motd & APT hooks, modprobe, XDG autostart, shell startup files, **PAM module dirs**, and **RAT/C2 spots** (system profile.d, **SSH-login rc & sshrc**, **network-event hooks**, module-autoload, udev RUN, binfmt, at jobs) — every command line also gets a **C2 tradecraft lens** (reverse-shell/backpipe/encoded-loader patterns, known-implant names, hardcoded callback IPs) |
